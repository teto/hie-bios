...
progParser = subparser
    ...
    <> command "config" (info (ConfigInfo <$> filepathParser) (progDesc "Print out the options that hie-bios thinks you will need to load a file."))
    <> command "cradle" (info (CradleInfo <$> filepathParser) (progDesc "."))
    <> command "root" (info (pure Root) (progDesc "Print out the options that hie-bios thinks you will need to load a file."))
